...
int main() {
    std::cout << "========================================\n";
    std::cout << " WizzMania Server - Testing Auth\n";
    std::cout << "========================================\n";
    
    // Test token generation
    int64_t test_user_id = 42;
    std::string token = Auth::generateToken(test_user_id);
    std::cout << "Generated token for user " << test_user_id << ":\n";
    std::cout << token << "\n\n";
    
    // Test token validation
    auto validated_user_id = Auth::validateToken(token);
    if (validated_user_id.has_value()) {
        std::cout << "✓ Token is valid! User ID: " << validated_user_id.value() << "\n";
    } else {
        std::cout << "✗ Token validation failed!\n";
    }
    
    // Test invalid token
    auto invalid = Auth::validateToken("invalid_token_here");
    if (!invalid.has_value()) {
        std::cout << "✓ Invalid token correctly rejected\n";
    }
    
    std::cout << "\n========================================\n";
    std::cout << " Auth tests completed!\n";
    std::cout << "========================================\n";
